...
- Codex：已完成 fix 复审；确认 `request_id`、配置容错、template API 已处置，但 worker 真实启动路径回归 + template 输出类型合同仍未闭环，handoff 当前为“部分处置”；另已补发 LLM DB 去业务化清理设计文档，待 Claude LLM 确认并按此收口
